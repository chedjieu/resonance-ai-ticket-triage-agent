...
def _fake_fallback_enabled() -> bool:
    """Auto-fallback to fake is opt-in only (MONK_ALLOW_FAKE_FALLBACK=1)."""
    return os.getenv("MONK_ALLOW_FAKE_FALLBACK", "").strip().lower() in (
        "1",
        "true",
        "yes",
    )
...
def activate_fake_fallback() -> None:
    ...
    os.environ["MONK_MODEL"] = "fake"
    os.environ["MONK_EMBEDDINGS"] = "fake"
    get_chat_model.cache_clear()
    get_embeddings.cache_clear()
    if not _fallback_logged:
        logger.warning(
            "Cloud model throttled (daily quota) — switched to MONK_MODEL=fake for this process. "
            "Wait for Bedrock quota reset, or use Vertex (gcloud auth application-default login)."
        )
        print(
            "\n!! Bedrock daily quota hit — continuing with MONK_MODEL=fake for this run.\n"
            "   Fix later: wait for quota reset, or set MONK_MODEL=google_vertexai:gemini-2.5-pro\n"
...
def invoke_with_throttle_fallback(fn: Callable[[], T]) -> T:
    ...
    Set MONK_ALLOW_FAKE_FALLBACK=0 to raise instead.
    """
    try:
        return fn()
    except Exception as exc:
        deny = os.getenv("MONK_ALLOW_FAKE_FALLBACK", "1").strip().lower() in (
            "0",
            "false",
            "no",
        )
...
def ensure_chat_model_available() -> bool:
    """Probe the configured chat model. Returns True only when MONK_MODEL=fake."""
